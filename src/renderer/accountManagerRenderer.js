...
const AccountManager = {
  ...
  async loadAccounts() {
    ...
    if (currentLoadVersion !== this.loadVersion) {
      console.log('账号列表加载结果已过期，跳过本次渲染');
      return;
    }
    ...
    const visibleAccounts = AccountDateFilterUtils.filterAccountsByScope(validAccounts, this.listScope);
    this.lastLoadedAccounts = validAccounts;
    
    console.log('开始渲染', visibleAccounts.length, '个账号');
    ...
    // 统计信息
    let totalCount = visibleAccounts.length;
    let activeCount = 0;
    let warningCount = 0;
    let expiredCount = 0;
    ...
    if (visibleAccounts.length === 0) {
      const emptyMessage = this.listScope === 'today'
        ? '今天没有新申请的账号，切到“全部”可以查看历史账号'
        : '暂无账号';
      html += `<div class="account-item" style="grid-column: 1 / -1; text-align:center; color:#999; padding:40px;">${emptyMessage}</div>`;
    }
    ...
    accountRows.forEach(row => {
      row.addEventListener('contextmenu', (e) => {
        ...
        const account = visibleAccounts.find(acc => acc.email === email);
        if (account) {
    ...
    this.updateListScopeUI(visibleAccounts.length, validAccounts.length);
  ...
  setListScope(scope = 'today') {
    const nextScope = scope === 'all' ? 'all' : 'today';
    this.listScope = nextScope;

    if (this.lastLoadedAccounts.length > 0) {
      const visibleCount = AccountDateFilterUtils.filterAccountsByScope(this.lastLoadedAccounts, this.listScope).length;
      this.updateListScopeUI(visibleCount, this.lastLoadedAccounts.length);
    }

    this.loadAccounts();
  },

  updateListScopeUI(visibleCount, totalCount) {
    const todayButton = document.getElementById('accountScopeTodayBtn');
    const allButton = document.getElementById('accountScopeAllBtn');
    const summary = document.getElementById('accountScopeSummary');

    if (todayButton) {
      todayButton.classList.toggle('active', this.listScope === 'today');
      todayButton.setAttribute('aria-pressed', this.listScope === 'today' ? 'true' : 'false');
    }

    if (allButton) {
      allButton.classList.toggle('active', this.listScope === 'all');
      allButton.setAttribute('aria-pressed', this.listScope === 'all' ? 'true' : 'false');
    }

    if (summary) {
      summary.textContent = AccountDateFilterUtils.getAccountDateFilterSummary(this.listScope, visibleCount, totalCount);
    }
  },
  ...
  showAccountContextMenu(event, account) {
    ...
    const menuHTML = `
      <div id="accountContextMenu" style="position: fixed; background: white; border: 1px solid #e5e5ea; border-radius: 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.12); z-index: 10000; min-width: 140px; visibility: hidden; padding: 4px 0;">
        ...
        <div class="context-menu-item" onclick="AccountManager.contextMenuEditPassword('${account.id}', '${this.escapeForInlineJs(account.email)}')">
        ...
        <div class="context-menu-divider"></div>
        <div class="context-menu-item" onclick="AccountManager.copyToClipboard('${this.escapeForInlineJs(account.email)}').then(() => { if(typeof showToast === 'function') showToast('邮箱已复制', 'success'); }); AccountManager.closeContextMenu();">
          <i data-lucide="mail" style="width: 13px; height: 13px;"></i>
          <span>复制邮箱</span>
        </div>
        <div class="context-menu-item" onclick="AccountManager.copyToClipboard('${this.escapeForInlineJs(account.password)}').then(() => { if(typeof showToast === 'function') showToast('密码已复制', 'success'); }); AccountManager.closeContextMenu();">
          <i data-lucide="key" style="width: 13px; height: 13px;"></i>
          <span>复制密码</span>
        </div>
        ${account.apiKey ? `
        <div class="context-menu-item" onclick="AccountManager.copyToClipboard('${this.escapeForInlineJs(account.apiKey)}').then(() => { if(typeof showToast === 'function') showToast('API Key 已复制', 'success'); }); AccountManager.closeContextMenu();">
          <i data-lucide="code" style="width: 13px; height: 13px;"></i>
          <span>复制 API Key</span>
        </div>
        ` : ''}
        ${account.refreshToken ? `
        <div class="context-menu-item" onclick="AccountManager.copyToClipboard('${this.escapeForInlineJs(account.refreshToken)}').then(() => { if(typeof showToast === 'function') showToast('Refresh Token 已复制', 'success'); }); AccountManager.closeContextMenu();">
          <i data-lucide="shield" style="width: 13px; height: 13px;"></i>
          <span>复制 Token</span>
        </div>
        ` : ''}
        <div class="context-menu-divider"></div>
        <div class="context-menu-item" style="color: #ff3b30;" onclick="AccountManager.contextMenuDelete('${account.id}', '${this.escapeForInlineJs(account.email)}')">
...
async function loadAccountsForSearch() {
  try {
    const result = await window.ipcRenderer.invoke('get-accounts');
    if (result.success && result.accounts) {
      const accounts = result.accounts.filter(acc => acc && acc.email);
      const scope = window.AccountManager && window.AccountManager.listScope
        ? window.AccountManager.listScope
        : 'today';
      cachedAccountsForSearch = AccountDateFilterUtils.filterAccountsByScope(accounts, scope);
    }
  } catch (error) {
