...
const TokenGetter = {
  ...
  // 判断账号的Token是否已过期
  // 如果账号有 idToken，则用 JWT exp 判断；否则退回到时间戳判断（1小时）
  isTokenExpired(account) {
    if (!account || !account.apiKey) return true;

    // 优先使用 JWT 过期检测（tokenUtils）
    if (account.idToken) {
      const { isTokenExpired: jwtExpired } = require('../services/tokenUtils');
      return jwtExpired(account.idToken, 60000);
    }
  ...
  async initAccountSwitcher() {
    try {
      const AccountSwitcher = require('./switcherRenderer');
