...
const AutoBindCard = {
  ...
  async loadUsageFromConfig() {
    try {
      ...
      console.log('加载配置文件:', result);
      const today = this.getTodayStr();
      const configData = (result && result.success && result.config) ? result.config : result;
      if (configData && configData.autoBindUsage && configData.autoBindUsage.date === today) {
        this.usageData = configData.autoBindUsage;
  ...
  async saveUsageToConfig() {
    try {
      const loadResult = await window.ipcRenderer.invoke('load-windsurf-config');
      let configData = {};
      if (loadResult && loadResult.success && loadResult.config && typeof loadResult.config === 'object') {
        configData = loadResult.config;
      }
      configData.autoBindUsage = this.usageData;
      const result = await window.ipcRenderer.invoke('save-windsurf-config', configData);
  ...
  generateCardInfo() {
    ...
    const fullYear = currentYear + yearOffset;
    const year = String(fullYear).slice(-2);
    // 月份: 如果是明年及以后，01-12 随机；如果恰好是今年+1年但仍需确保在未来
  ...
  async onViewSwitch() {
    ...
    if (window.ipcRenderer) {
      window.ipcRenderer.removeAllListeners('auto-fill-log');
...
// 初始化（延迟到 renderer.js 挂载后）
// AutoBindCard.init() 将在 renderer.js 中调用
